...
class AppointmentRefuseAdapter(
    private val onDetailClick: (Int) -> (Unit)
): ListAdapter<AppointmentChild, AppointmentRefuseAdapter.ViewHolder>(AppointmentDiffCallback()) {
    override fun onCreateViewHolder(holder: ViewGroup, position: Int): AppointmentRefuseAdapter.ViewHolder {
        val view = LayoutInflater.from(holder.context).inflate(R.layout.item_refuse_receive_appointment, holder, false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: AppointmentRefuseAdapter.ViewHolder, position: Int) {
