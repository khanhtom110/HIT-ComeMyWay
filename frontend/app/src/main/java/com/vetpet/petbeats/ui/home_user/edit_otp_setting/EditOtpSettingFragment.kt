...
class EditOtpSettingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEditOtpSettingBinding.inflate(inflater, container, false)
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.event.collect { event ->
                    when (event) {
                        ...
                        is EditOtpSettingEvent.NavigationForgotpasswordSetting -> {
                            findNavController().navigate(R.id.editOtp_editForgotpassword)
                        }

                        //Nút verify chuyển sang màn tiếp theo
                        is EditOtpSettingEvent.NavigationSendToken -> {
                            findNavController().navigate(
                                R.id.editResetPasswordSettingFragment,
                                Bundle().apply {
