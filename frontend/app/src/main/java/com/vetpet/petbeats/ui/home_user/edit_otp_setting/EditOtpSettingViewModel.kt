...
@HiltViewModel
class EditOtpSettingViewModel @Inject constructor(
    private val repository: HomeUserRepository
): ViewModel() {
    private val _state = MutableStateFlow(EditOtpSettingState())
    val state = _state.asStateFlow()

    private val _event = MutableSharedFlow<EditOtpSettingEvent>()
    val event = _event.asSharedFlow()

    fun vectorClick() {
        viewModelScope.launch {
            _event.emit(EditOtpSettingEvent.NavigationForgotpasswordSetting)
        }
    }
    ...
    fun onResetClick(email: String) {
        viewModelScope.launch {
            ...
            val result = repository.resetOtpUser(request)
            ...
            when (result) {
                is DataResult.Success -> {
                    ...
                    _event.emit(EditOtpSettingEvent.NavigationSendToken(refreshToken, email))
                }
