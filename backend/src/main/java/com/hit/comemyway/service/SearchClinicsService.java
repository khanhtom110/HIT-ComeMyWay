...
public class SearchClinicsService {
    ...
    public List<Double> calculateDistance(Double userLatitude, Double userLongitude, List<Clinic> clinics) {
        ...
        String link = "http://router.project-osrm.org/table/v1/driving/"
                + stringBuilder + "?sources=0&annotations=distance";
        ...
        try {
            ...
            result.clear();
            for (Clinic clinic : clinics) {
                double fallback = calculateFallback(userLatitude, userLongitude,
    ...
    public double calculateFallback(Double userLatitude, Double userLongitude, Double clinicLatitude, Double clinicLongitude) {
        double distance = calculateAirDistance(userLatitude, userLongitude, clinicLatitude, clinicLongitude);

        // He so duong vong o Ha Noi la 1.3
        return distance * 1.3;
    }

    public double calculateAirDistance(Double userLatitude, Double userLongitude, Double clinicLatitude, Double clinicLongitude) {
