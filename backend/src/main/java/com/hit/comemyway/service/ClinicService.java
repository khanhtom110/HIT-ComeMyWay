...
public class ClinicService {
    ...
    public List<ClinicSearchResponse> findClinics(String keyword, Double userLatitude, Double userLongitude, Double radius, int limit) {
        List<Clinic> clinics;
    ...
    private List<Clinic> fetchClinicsWithLocation(String keyword, Double userLatitude, Double userLongitude, Double radius) {
        ...
        List<Clinic> clinics = clinicRepository.findClinicsWithLocation(keyword, minLatitude, maxLatitude, minLongitude, maxLongitude);

        if (clinics.size() > NUMBER_OSRM) {
            clinics.sort((c1, c2) -> {
                double dist1 = searchClinicsService.calculateAirDistance(userLatitude, userLongitude, c1.getLatitude(), c1.getLongitude());
                double dist2 = searchClinicsService.calculateAirDistance(userLatitude, userLongitude, c2.getLatitude(), c2.getLongitude());
    ...
    private List<ClinicSearchResponse> mapToResponses(List<Clinic> clinics, Double userLatitude, Double userLongitude) {
        ...
        for (int i = 0; i < clinics.size(); i++) {
            ...
            responses.add(new ClinicSearchResponse(
                    clinic.getId(),
                    clinic.getThumbnailUrl(),
                    clinic.getName(),
                    isOperating(clinic, now),
                    clinic.getRating(),
                    distance,
                    clinic.getAddress(),
                    clinic.getOpenTime(),
                    ...
                    clinic.getServices().stream()
                            .map(service -> service.getName()).toList()
            ));
        }
        ...
        if (userLatitude != null && userLongitude != null) {
            responses.sort(Comparator
                    .comparing(ClinicSearchResponse::distance, Comparator.nullsLast(Comparator.naturalOrder())));
        }
        return responses;
    }

    private boolean isOperating(Clinic clinic, LocalTime now) {
        return clinic.getOpenTime() != null
                && clinic.getCloseTime() != null
                && now.isAfter(clinic.getOpenTime())
                && now.isBefore(clinic.getCloseTime());
    }
